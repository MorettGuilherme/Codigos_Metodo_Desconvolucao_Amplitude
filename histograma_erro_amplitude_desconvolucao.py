...
# Objetivo do código: plote do histograma do erro absoluto da amplitude pelo método de Desconvolução de Sinal para o caso P = N.
...
1) Função para o cálculo da estatística do erro de estimação da amplitude pelo método de Desconvolução de Sinal.
...
2) Instrução para o plote do histograma do tipo A do erro de estimação da amplitude pelo método de Desconvolução de Sinal.
Entrada: lista com os erros de estimação da amplitude e seus dados estatísticos.
Saída: nada.

3) Instrução para o plote do histograma do tipo B do erro de estimação da amplitude pelo método de Desconvolução de Sinal.
Entrada: número de ocupação, lista com os erros de estimação da amplitude e seus dados estatísticos para os janelamento 7, 15 e 19.
Saída: nada.
...
titulo_programa = colored("Análise do erro de estimação da amplitude pelo método de Desconvolução de Sinal para o caso P = N:\n", attrs=["bold"])

# Impressão do título do programa.
print(titulo_programa)

### --------- 1) FUNÇÃO PARA O CÁLCULO DOS DADOS ESTATÍSTICOS DO ERRO DE ESTIMAÇÃO DA AMPLITUDE PARA O MÉTODO DE DESCONVOLUÇÃO DE SINAL -------- ###
...
### ----------- 2) INSTRUÇÃO PARA A CONSTRUÇÃO DO HISTOGRAMA DO TIPO A DO ERRO DE ESTIMAÇÃO DA AMPLITUDE PELO MÉTODO DE DESCONVOLUÇÃO DE SINAL ----------- ###

# Definição da instrução para a confecção do histograma do tipo A do erro de estimação da amplitude pelo método de Desconvolução de Sinal.
def histograma_A_erro_estimacao_amplitude_desconvolucao(n_ocupacao, lista_erro_amplitude, media_erro_amplitude, var_erro_amplitude, desvio_padrao_erro_amplitude):
...
# A variável n_ocupacao armazena o valor digitado da ocupação desejada no terminal pelo usuário.
    ...
    if tipo_histograma == "A":
        ...
        lista_erro_estimacao_amplitude = metodo_desconvolucao_P_igual_N(n_janelamento, Matriz_Pulsos_Sinais_Treino_Janelado, vetor_amplitude_referencia_teste_janelado)
        ...
        histograma_A_erro_estimacao_amplitude_desconvolucao(n_ocupacao, lista_erro_estimacao_amplitude, media_erro_estimacao_amplitude, var_erro_estimacao_amplitude, desvio_padrao_erro_estimacao_amplitude)
    ...
    # Caso a variável tipo_histograma seja "B".
        ...
        lista_erro_estimacao_amplitude_J7 = metodo_desconvolucao_P_igual_N(n_janelamento_J7, Matriz_Pulsos_Sinais_Janelado_J7, vetor_amplitude_referencia_janelado_J7)
        media_erro_estimacao_amplitude_J7, var_erro_estimacao_amplitude_J7, desvio_padrao_erro_estimacao_amplitude_J7 = dados_estatisticos_erro_estimacao_amplitude_desconvolucao(lista_erro_estimacao_amplitude_J7)
    
        lista_erro_estimacao_amplitude_J15 = metodo_desconvolucao_P_igual_N(n_janelamento_J15, Matriz_Pulsos_Sinais_Janelado_J15, vetor_amplitude_referencia_janelado_J15)
        media_erro_estimacao_amplitude_J15, var_erro_estimacao_amplitude_J15, desvio_padrao_erro_estimacao_amplitude_J15 = dados_estatisticos_erro_estimacao_amplitude_desconvolucao(lista_erro_estimacao_amplitude_J15)
    
        lista_erro_estimacao_amplitude_J19 = metodo_desconvolucao_P_igual_N(n_janelamento_J19, Matriz_Pulsos_Sinais_Janelado_J19, vetor_amplitude_referencia_janelado_J19)
        media_erro_estimacao_amplitude_J19, var_erro_estimacao_amplitude_J19, desvio_padrao_erro_estimacao_amplitude_J19 = dados_estatisticos_erro_estimacao_amplitude_desconvolucao(lista_erro_estimacao_amplitude_J19)
    
        histograma_B_erro_estimacao_amplitude_BLUE2(n_ocupacao, lista_erro_estimacao_amplitude_J7, media_erro_estimacao_amplitude_J7, var_erro_estimacao_amplitude_J7, desvio_padrao_erro_estimacao_amplitude_J7, lista_erro_estimacao_amplitude_J15, media_erro_estimacao_amplitude_J15, var_erro_estimacao_amplitude_J15, desvio_padrao_erro_estimacao_amplitude_J15, lista_erro_estimacao_amplitude_J19, media_erro_estimacao_amplitude_J19, var_erro_estimacao_amplitude_J19, desvio_padrao_erro_estimacao_amplitude_J19)
